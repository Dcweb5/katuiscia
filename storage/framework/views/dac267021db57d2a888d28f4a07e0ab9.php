<?php $__env->startSection('title', 'Gestion des Collections'); ?>
...
<div class="dashboard-content">
  ...
  <?php if(session('success')): ?>
    <script>document.addEventListener('DOMContentLoaded',function(){showToast(<?php echo json_encode(session('success'), 15, 512) ?>,'success');});</script>
  <?php endif; ?>

  <div class="stats-grid" style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:var(--space-xl);">
    <div class="card stat-card"><span class="stat-title">Actives</span><span class="stat-value"><?php echo e($active); ?></span></div>
    <div class="card stat-card"><span class="stat-title">Brouillons</span><span class="stat-value"><?php echo e($drafts); ?></span></div>
    <div class="card stat-card"><span class="stat-title">Total</span><span class="stat-value"><?php echo e($collections->count()); ?></span></div>
    <div class="card stat-card"><span class="stat-title">Produits</span><span class="stat-value"><?php echo e($collections->sum(fn($c) => $c->products->count())); ?></span></div>
  </div>
        ...
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:12px;">
              ...
              <img src="<?php echo e($collection->image_url); ?>" style="width:48px;height:36px;border-radius:6px;object-fit:cover;">
              <?php else: ?>
              ...
              <div>
                ...
                <span style="font-size:11px;color:var(--color-success);display:block;">-<?php echo e($collection->discount_percent); ?>%</span>
                <?php endif; ?>
              </div>
            </div>
          </td>
          <td style="font-size:var(--text-sm);"><?php echo e($collection->products->count()); ?> produit(s)</td>
          <td>
            <?php if($collection->original_price && $collection->original_price > $collection->price): ?>
            <span style="text-decoration:line-through;color:var(--color-text-muted);font-size:12px;"><?php echo e(number_format($collection->original_price, 0, ',', ' ')); ?> €</span>
            <?php endif; ?>
            <strong><?php echo e(number_format($collection->price, 0, ',', ' ')); ?> €</strong>
          </td>
          <td style="font-size:var(--text-sm);"><?php echo e($collection->category?->name ?? '—'); ?></td>
          <td>
            <span style="font-size:11px;font-weight:600;color:#fff;padding:3px 10px;border-radius:var(--radius-full);background:<?php echo e($collection->is_active ? 'var(--color-success)' : 'var(--color-text-muted)'); ?>;"><?php echo e($collection->is_active ? 'Actif' : 'Brouillon'); ?></span>
          </td>
          <td>
            <div style="display:flex;gap:4px;">
              <button class="action-btn" title="Modifier" onclick="openEditModal(<?php echo e($collection->id); ?>)">✏️</button>
              <form method="POST" action="<?php echo e(route('admin.collections.toggle', $collection)); ?>" style="display:inline;">
                <?php echo csrf_field(); ?> <?php echo method_field('PUT'); ?>
                <button type="submit" class="action-btn" title="<?php echo e($collection->is_active ? 'Désactiver' : 'Activer'); ?>"><?php echo e($collection->is_active ? '👁' : '👁‍🗨'); ?></button>
              </form>
              <form method="POST" action="<?php echo e(route('admin.collections.destroy', $collection)); ?>" style="display:inline;" onsubmit="return confirm('Supprimer cette collection ?')">
...
<dialog id="collection-modal" style="border:none;border-radius:12px;padding:0;box-shadow:0 20px 60px rgba(0,0,0,0.2);max-width:700px;width:95%;">
  <div style="padding:0;">
    ...
    <form id="collection-form" method="POST" action="<?php echo e(route('admin.collections.store')); ?>" enctype="multipart/form-data" style="padding:1.5rem;max-height:75vh;overflow-y:auto;">
      <?php echo csrf_field(); ?>
      ...
      <div class="admin-form-group">
        ...
        <div id="products-list" style="max-height:220px;overflow-y:auto;border:1px solid var(--color-border);border-radius:8px;padding:0.5rem;">
          ...
          <label style="display:flex;align-items:center;gap:0.75rem;padding:8px 10px;cursor:pointer;border-radius:6px;font-size:14px;" class="product-check-label">
            <input type="checkbox" name="product_ids[]" value="<?php echo e($product->id); ?>" class="product-check">
            <img src="<?php echo e($product->image_url); ?>" style="width:40px;height:40px;border-radius:6px;object-fit:cover;flex-shrink:0;" onerror="this.style.display='none'">
...
<script>
var collectionsData = <?php echo json_encode($collections->load('products'), 15, 512) ?>;

var productsPrices = {};
<?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
productsPrices[<?php echo e($p->id); ?>] = <?php echo e($p->price); ?>;
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
function openCreateModal() {
  ...
  document.getElementById('collection-form').action = '<?php echo e(route('admin.collections.store')); ?>';
...
function openEditModal(id) {
  ...
  document.getElementById('collection-form').action = '<?php echo e(url('admin/collections')); ?>/' + c.id;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\mes sites web\KATUISCIA LARAVEL\resources\views\admin\collections\index.blade.php ENDPATH**/ ?>