<?php $__env->startSection('title', 'Mes Récompenses'); ?>
...
<div class="dashboard-content">
  <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:var(--space-xl);">
    ...
    <div style="background:var(--color-peach); padding:var(--space-sm) var(--space-lg); border-radius:var(--radius-full); font-size:var(--text-xs); font-weight:600; color:var(--color-dark); letter-spacing:0.1em; text-transform:uppercase;">
      <?php echo e(auth()->user()->loyalty_points >= 1500 ? '✨ Or' : '🌱 Essentiel'); ?>
    ...
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:var(--space-2xl); align-items:center;">
      ...
      <div>
        ...
        <div style="display:flex; align-items:baseline; gap:var(--space-sm);">
          <span style="font-size:5rem; font-family:var(--font-display); line-height:1; font-weight:300;" id="points-counter"><?php echo e(number_format($points, 0, ',', ' ')); ?></span>
          <span style="font-size:var(--text-lg); opacity:0.7; text-transform:uppercase; letter-spacing:0.1em;">pts</span>
        </div>
        <p style="font-size:var(--text-sm); opacity:0.6; margin-top:var(--space-sm);">Soit environ <?php echo e(round($points / 1000 * 10)); ?> € en réduction</p>
      </div>
      ...
      <div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:flex-end; gap:var(--space-md); margin-bottom:var(--space-xs);">
          <span style="font-size:var(--text-sm); font-weight:500; opacity:0.9;"><?php echo e($points >= 1500 ? '✨ Statut Or' : '🌱 Essentiel'); ?></span>
          <span style="font-size:var(--text-sm); font-weight:500; opacity:0.9;">⭐ <?php echo e($nextTier['name']); ?></span>
        </div>
        <div style="width:100%; height:10px; background:rgba(255,255,255,0.15); border-radius:5px; overflow:hidden;">
          <div style="width:<?php echo e($progress); ?>%; height:100%; background:linear-gradient(90deg, rgba(255,255,255,0.6), white); border-radius:5px; transition:width 1s ease;"></div>
        </div>
        <div style="display:flex; justify-content:space-between; font-size:11px; margin-top:6px; opacity:0.6;">
          <span><?php echo e(number_format($points, 0, ',', ' ')); ?></span>
          <span><?php echo e(number_format($nextTier['pts'], 0, ',', ' ')); ?></span>
        </div>
        <?php if($remaining > 0): ?>
        <p style="text-align:center; font-size:var(--text-xs); margin-top:var(--space-sm); opacity:0.8;">
          🔥 Plus que <strong><?php echo e(number_format($remaining, 0, ',', ' ')); ?> pts</strong> pour le statut <?php echo e($nextTier['name']); ?> !
        </p>
        <?php else: ?>
        <p style="text-align:center; font-size:var(--text-xs); margin-top:var(--space-sm); opacity:0.8;">
  ...
  <div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:var(--space-lg); margin-bottom:var(--space-2xl);">
    ...
    <div class="card" style="border:2px solid <?php echo e($reward['available'] ? 'var(--color-warm)' : 'var(--color-border)'); ?>; padding:var(--space-xl); <?php echo e(!$reward['available'] ? 'opacity:0.6;' : ''); ?>">
      <div style="display:flex; align-items:center; gap:var(--space-md); margin-bottom:var(--space-md);">
        <span style="font-size:32px;"><?php echo e($reward['icon']); ?></span>
        <div>
          <strong style="font-size:var(--text-lg); color:var(--color-dark); display:block;"><?php echo e($reward['title']); ?></strong>
          <span style="font-size:var(--text-sm); color:var(--color-warm); font-weight:600;"><?php echo e(number_format($reward['pts'], 0, ',', ' ')); ?> pts</span>
        </div>
      </div>
      <p style="color:var(--color-text-muted); font-size:var(--text-sm); margin-bottom:var(--space-lg); line-height:1.5;"><?php echo e($reward['desc']); ?></p>
      <?php if($reward['available']): ?>
      ...
      <div style="width:100%; padding:12px 0; text-align:center; font-size:var(--text-xs); color:var(--color-text-muted); text-transform:uppercase; letter-spacing:0.1em;">
        <div style="width:100%; height:4px; background:var(--color-gray-medium); border-radius:2px; margin-bottom:6px;">
          <div style="width:<?php echo e(min(100, round($points / $reward['pts'] * 100))); ?>%; height:100%; background:var(--color-warm); border-radius:2px;"></div>
        </div>
        <?php echo e(round($points / $reward['pts'] * 100)); ?>% — Encore <?php echo e(number_format($reward['pts'] - $points, 0, ',', ' ')); ?> pts
      </div>
      <?php endif; ?>
    </div>
...
<?php echo $__env->make('layouts.account', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\mes sites web\KATUISCIA LARAVEL\resources\views\account\rewards.blade.php ENDPATH**/ ?>