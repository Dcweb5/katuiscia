<?php $__env->startSection('title', 'Contact — KATUISCIA'); ?>
<?php $__env->startSection('description', 'Contactez l\'équipe KATUISCIA. Formulaire de contact, adresse, horaires et réseaux sociaux.'); ?>
...
<div class="max-w-site mx-auto px-4 lg:px-8 pb-24 grid grid-cols-1 lg:grid-cols-[1.2fr_1fr] gap-16">
  ...
  <div class="reveal-k-k">
    <form method="POST" action="<?php echo e(route('contact.store')); ?>" class="bg-white rounded-2xl p-8 shadow-card" style="border:1px solid #ede4db;">
      <?php echo csrf_field(); ?>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          <input type="text" name="name" value="<?php echo e(old('name')); ?>" class="contact-input" placeholder="Votre nom" required>
        </div>
        <div>
          <label class="contact-label">Email *</label>
          <input type="email" name="email" value="<?php echo e(old('email')); ?>" class="contact-input" placeholder="votre@email.com" required>
        </div>
      </div>
      ...
      <div style="margin:1.25rem 0;">
        ...
        <select name="subject" class="contact-input" style="appearance:auto;">
          ...
          <option value="commande" <?php if(old('subject')=='commande'): echo 'selected'; endif; ?>>Question sur une commande</option>
          <option value="produit" <?php if(old('subject')=='produit'): echo 'selected'; endif; ?>>Information produit</option>
          <option value="retour" <?php if(old('subject')=='retour'): echo 'selected'; endif; ?>>Retour / Remboursement</option>
          <option value="partenariat" <?php if(old('subject')=='partenariat'): echo 'selected'; endif; ?>>Partenariat / Grossiste</option>
          <option value="formation" <?php if(old('subject')=='formation'): echo 'selected'; endif; ?>>Formation</option>
          <option value="presse" <?php if(old('subject')=='presse'): echo 'selected'; endif; ?>>Presse</option>
          <option value="autre" <?php if(old('subject')=='autre'): echo 'selected'; endif; ?>>Autre</option>
        </select>
      </div>

      <div style="margin-bottom:1.5rem;">
        <label class="contact-label">Message *</label>
        <textarea name="message" rows="5" class="contact-input" style="resize:vertical;" placeholder="Votre message..." required><?php echo e(old('message')); ?></textarea>
      </div>
...
<script>
  document.getElementById('contact-toast').textContent = <?php echo json_encode(session('success'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.public', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\mes sites web\KATUISCIA LARAVEL\resources\views\pages\contact.blade.php ENDPATH**/ ?>