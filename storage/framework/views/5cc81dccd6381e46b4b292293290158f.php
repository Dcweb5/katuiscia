...
<div class="dashboard-content">
  <div style="display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:var(--space-2xl);flex-wrap:wrap;gap:1rem;">
    ...
    <div style="display:flex;gap:0.5rem;">
      <a href="?period=month" class="btn-primary" style="font-size:12px;text-decoration:none;<?php echo e($period=='month'?'':'background:transparent;color:var(--color-text);border-color:var(--color-border);'); ?>">Ce mois</a>
      <a href="?period=quarter" class="btn-primary" style="font-size:12px;text-decoration:none;<?php echo e($period=='quarter'?'':'background:transparent;color:var(--color-text);border-color:var(--color-border);'); ?>">Trimestre</a>
      <a href="?period=year" class="btn-primary" style="font-size:12px;text-decoration:none;<?php echo e($period=='year'?'':'background:transparent;color:var(--color-text);border-color:var(--color-border);'); ?>">Année</a>
    </div>
  </div>
  ...
  <div class="stats-grid" style="display:grid;grid-template-columns:repeat(5,1fr);gap:1rem;margin-bottom:var(--space-xl);">
    <div class="card stat-card" style="padding:1.25rem;">
      ...
      <span class="stat-value" style="font-family:var(--font-display);"><?php echo e(number_format($totalRevenue, 0, ',', ' ')); ?> €</span>
    </div>
    <div class="card stat-card" style="padding:1.25rem;">
      <span class="stat-title">Marge brute</span>
      <span class="stat-value"><?php echo e(number_format($margin, 0, ',', ' ')); ?> €</span>
      <span style="font-size:11px;color:var(--color-success);">~60%</span>
    </div>
    <div class="card stat-card" style="padding:1.25rem;">
      <span class="stat-title">Panier moyen</span>
      <span class="stat-value"><?php echo e(number_format($avgBasket, 0, ',', ' ')); ?> €</span>
    </div>
    <div class="card stat-card" style="padding:1.25rem;">
    ...
    <div class="card stat-card" style="padding:1.25rem;">
      ...
      <span class="stat-value" style="color:var(--color-error);"><?php echo e(number_format($refunds, 0, ',', ' ')); ?> €</span>
    </div>
  </div>
...
new Chart(document.getElementById('monthlyChart'), {
  type: 'bar',
  data: {
    labels: <?php echo json_encode($monthly->pluck('m')->toArray(), 15, 512) ?>,
    datasets: [{
      label: 'CA (€)',
      data: <?php echo json_encode($monthly->pluck('total')->toArray(), 15, 512) ?>,
...
var statusData = <?php echo json_encode($byStatus->pluck('count', 'status'), 512) ?>;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\mes sites web\KATUISCIA LARAVEL\resources\views\admin\finances\index.blade.php ENDPATH**/ ?>