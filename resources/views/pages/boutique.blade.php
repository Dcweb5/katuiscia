...
<section class="max-w-site mx-auto px-4 lg:px-8 pb-12 reveal-k-k delay-1">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
    ...
    <a href="{{ route('collection.show', $collection->slug) }}" style="text-decoration:none;color:inherit;display:block;border-radius:14px;overflow:hidden;background:#fff;border:1px solid #ede4db;transition:all 0.25s;" class="reveal-k-k" onmouseover="this.style.transform='translateY(-3px)';this.style.boxShadow='0 12px 32px rgba(0,0,0,0.06)'" onmouseout="this.style.transform='';this.style.boxShadow=''">
      <div style="position:relative;height:220px;overflow:hidden;">
        ...
        <span style="position:absolute;top:12px;right:12px;background:#c62828;color:#fff;font-size:11px;font-weight:700;padding:4px 10px;border-radius:99px;">-{{ $collection->discount_percent }}%</span>
        @endif
      </div>
      <div style="padding:1.25rem;">
        ...
        <div style="display:flex;align-items:baseline;gap:0.5rem;">
          ...
          <span style="font-size:14px;text-decoration:line-through;color:#a39688;">{{ number_format($collection->original_price, 0, ',', ' ') }} €</span>
          @endif
          <span style="font-family:var(--font-display);font-size:1.25rem;color:#2d2117;">{{ number_format($collection->price, 0, ',', ' ') }} €</span>
        </div>
