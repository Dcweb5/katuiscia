...
<div class="dashboard-content">
  @php $pts = auth()->user()->loyalty_points ?? 0; @endphp
  ...
  <div class="hero-welcome">
    <div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:var(--space-xl);">
      ...
      <div class="points-orb">
        <span style="font-size:1.6rem; font-family:var(--font-display); font-weight:600; line-height:1;">{{ $pts > 999 ? floor($pts/1000).'k' : $pts }}</span>
  ...
  <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:var(--space-md); margin-bottom:var(--space-xl);">
    <div class="stat-icon-card">
      ...
      <div>
        ...
        <strong style="font-size:var(--text-lg); color:var(--color-dark); display:block;">{{ number_format($pts, 0, ',', ' ') }} pts</strong>
        <span style="font-size:10px; color:var(--color-warm);">{{ $pts >= 1500 ? 'Or ⭐' : 'Essentiel 🌱' }}</span>
      </div>
    </div>
    <div class="stat-icon-card">
    ...
    <div class="stat-icon-card">
      ...
      <div>
        ...
        <strong style="font-size:var(--text-lg); color:var(--color-dark); display:block;">{{ $pts >= 500 ? 1 : 0 }}</strong>
  ...
  <div style="display:grid; grid-template-columns:1fr 1fr; gap:var(--space-xl); margin-bottom:var(--space-xl);">
    ...
    <div class="card" style="background:linear-gradient(145deg, var(--color-peach), rgba(196,150,122,0.08)); padding:var(--space-xl);">
      ...
      @php $progress = min(100, round($pts / 1500 * 100)); $remaining = max(0, 1500 - $pts); @endphp
      <div style="display:flex; justify-content:space-between; margin-bottom:var(--space-xs); font-size:var(--text-sm);">
        <span>🌱 Essentiel</span><span>{{ $pts >= 1500 ? '✅' : '🎯' }} Or (1500 pts)</span>
      </div>
      <div style="width:100%; height:12px; background:rgba(255,255,255,0.6); border-radius:6px; overflow:hidden;">
        <div class="progress-fill" data-progress="{{ $progress }}" style="width:0%; height:100%; background:linear-gradient(90deg, var(--color-warm), var(--color-dark)); border-radius:6px; transition:width 1.5s ease;"></div>
      </div>
      <p style="text-align:right; font-size:var(--text-xs); color:var(--color-text-muted); margin-top:var(--space-xs);">{{ number_format($pts, 0, ',', ' ') }} / 1 500 pts</p>
      @if($remaining > 0)
      <p style="text-align:center; margin-top:var(--space-md); font-size:var(--text-sm); color:var(--color-dark);">🔥 Plus que <strong>{{ number_format($remaining, 0, ',', ' ') }} pts</strong> pour le statut Or !</p>
      @endif
      <a href="{{ url('compte/recompenses') }}" class="btn-katuiscia" style="display:block; text-align:center; margin-top:var(--space-md);">Voir mes récompenses</a>
    </div>
