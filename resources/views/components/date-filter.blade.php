<div class="date-filter" style="position:relative;">
  <button type="button" class="filter-pill {{ request()->anyFilled(['date_from','date_to']) ? 'active' : '' }}" onclick="this.nextElementSibling.classList.toggle('open')" style="display:flex;align-items:center;gap:6px;">
  ...
  <div class="date-dropdown" style="display:none;position:absolute;top:100%;left:0;margin-top:6px;background:#fff;border:1px solid var(--color-border);border-radius:12px;padding:1rem;box-shadow:0 8px 30px rgba(0,0,0,0.1);z-index:100;white-space:nowrap;">
    <div style="display:flex;gap:0.75rem;align-items:center;">
      <div>
        <label style="font-size:11px;color:var(--color-text-muted);display:block;margin-bottom:4px;">Du</label>
        <input type="date" name="date_from" form="{{ $formId ?? 'filter-form' }}" value="{{ request('date_from') }}" class="admin-input" style="padding:6px 10px;font-size:13px;width:140px;">
      </div>
      ...
      <div>
        ...
        <input type="date" name="date_to" form="{{ $formId ?? 'filter-form' }}" value="{{ request('date_to') }}" class="admin-input" style="padding:6px 10px;font-size:13px;width:140px;">
      </div>
...
<style>
.date-dropdown.open { display:block !important; }
</style>
