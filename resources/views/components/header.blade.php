...
<header class="header-k fixed top-0 left-0 right-0 z-[310] bg-cream transition-all duration-300 ease-out-expo">
  <div class="max-w-site mx-auto px-4 lg:px-8 flex flex-col justify-end h-[190px] pb-5">
    ...
    <div class="grid grid-cols-[1fr_auto_1fr] items-center w-full flex-1 mb-[30px]">
      ...
      <!-- Search -->
      <form action="{{ url('recherche') }}" method="GET" class="hidden lg:flex items-center border border-dark/80 rounded-[10px] overflow-hidden h-12 w-[339px]">
        <input type="text" name="q" placeholder="Rechercher" aria-label="Rechercher"
               class="bg-transparent px-4 font-body text-sm text-dark w-full outline-none placeholder:text-text-muted" value="{{ request('q', '') }}">
      ...
      <div class="flex items-center justify-center relative z-10">
        <a href="{{ url('/') }}" class="flex items-center justify-center">
          <img src="{{ asset('assets/images/K LOGO.png') }}" alt="KATUISCIA" class="h-11 w-auto object-contain hidden lg:block">
          <img src="{{ asset('assets/images/K ICONE.png') }}" alt="KATUISCIA" class="h-16 w-auto object-contain lg:hidden">
        </a>
      </div>
      ...
      <div class="flex items-center justify-end gap-6">
        @guest
        <a href="{{ url('connexion') }}" class="flex items-center justify-center w-11 h-11 text-dark rounded-full hover:text-warm transition-colors relative" aria-label="Se connecter">
        ...
        @else
        <a href="{{ url('compte') }}" class="flex items-center justify-center w-11 h-11 text-dark rounded-full hover:text-warm transition-colors relative" aria-label="Mon compte">
          <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
        </a>
        @endguest
        @auth
    ...
    <div class="hidden lg:flex items-center justify-between w-full pb-1 border-b border-border-k">
      ...
      <nav class="flex items-center gap-10 flex-1 pr-[150px]" aria-label="Navigation gauche">
        <a href="{{ url('boutique') }}" class="nav-link-k">SOINS</a>
        <a href="{{ url('maison') }}" class="nav-link-k">MAISON</a>
        ...
        <a href="{{ url('contact') }}" class="nav-link-k">CONTACT</a>
      </nav>
      ...
      <nav class="flex items-center gap-10 flex-1 justify-end pl-[150px]" aria-label="Navigation droite">
        <div class="dropdown-k group relative">
          ...
          <div class="dropdown-panel-k">
            ...
            <a href="{{ url('grossiste') }}">Devenir Grossiste</a>
            <a href="{{ url('avantage-en-ligne') }}">Avantage en ligne</a>
            <a href="{{ url('contact') }}">Où nous trouver ?</a>
          </div>
        </div>
        <div class="dropdown-k group relative">
          ...
          <div class="dropdown-panel-k min-w-[360px]">
            ...
            <a href="{{ url('valeurs-beaute-responsable') }}">À propos de la beauté responsable</a>
            <a href="{{ url('valeurs-emballage-durable') }}">À propos de l'emballage durable</a>
            <a href="{{ url('valeurs-personne-biodiversite') }}">À propos de la personne &amp; la biodiversité</a>
            ...
            <a href="{{ url('maison') }}">Un avenir plus beau</a>
          </div>
        </div>
      </nav>
    </div>
...
<div class="mobile-menu-k fixed top-0 right-full w-full h-screen bg-cream z-[300] flex flex-col items-start justify-start pt-[120px] px-12 pb-24 gap-8 overflow-y-auto transition-all duration-500 ease-out-expo">
  <!-- Search -->
  <form action="{{ url('recherche') }}" method="GET" class="flex items-center border border-dark/80 rounded-[10px] overflow-hidden h-12 w-full mb-4">
    <input type="text" name="q" placeholder="Rechercher" aria-label="Rechercher"
           class="bg-transparent px-4 font-body text-sm text-dark w-full outline-none" value="{{ request('q', '') }}">
  ...
  <a href="{{ url('/') }}" class="mobile-link-k">Accueil</a>
  <a href="{{ url('boutique') }}" class="mobile-link-k">Soins</a>
  <a href="{{ url('maison') }}" class="mobile-link-k">Maison</a>
  ...
  <div class="w-full">
    ...
    <div class="hidden ml-4 mt-4 flex flex-col gap-3">
      <a href="{{ url('blog') }}" class="text-sm text-text-light hover:text-dark transition-colors">Blog</a>
      <a href="{{ url('diagnostic') }}" class="text-sm text-text-light hover:text-dark transition-colors">Diagnostic</a>
    </div>
  </div>
  ...
  <div class="w-full">
    ...
    <div class="hidden ml-4 mt-4 flex flex-col gap-3">
      ...
      <a href="{{ url('valeurs-beaute-responsable') }}" class="text-sm text-text-light hover:text-dark transition-colors">Beauté responsable</a>
      <a href="{{ url('valeurs-emballage-durable') }}" class="text-sm text-text-light hover:text-dark transition-colors">Emballage durable</a>
      <a href="{{ url('valeurs-personne-biodiversite') }}" class="text-sm text-text-light hover:text-dark transition-colors">Personne &amp; biodiversité</a>
      <span class="text-md font-bold text-warm uppercase tracking-wider mt-2">Notre Vision</span>
      <a href="{{ url('maison') }}" class="text-sm text-text-light hover:text-dark transition-colors">Un avenir plus beau</a>
    </div>
  </div>

  <a href="{{ url('contact') }}" class="mobile-link-k">Contact</a>

  @guest
  <a href="{{ url('connexion') }}" class="mobile-link-k">Connexion</a>
  <a href="{{ url('inscription') }}" class="mobile-link-k">Inscription</a>
  @else
  <a href="{{ url('compte') }}" class="mobile-link-k">Mon Compte</a>
  <a href="{{ url('admin') }}" class="mobile-link-k" style="color:var(--color-warm);">Administration</a>
  <form method="POST" action="{{ url('deconnexion') }}" style="display:inline;">
    @csrf
  ...
  <div class="w-full">
    ...
    <div class="hidden ml-4 mt-4 flex flex-col gap-3">
      <a href="{{ url('formation') }}" class="text-sm text-text-light hover:text-dark transition-colors">Formation</a>
      <a href="{{ url('grossiste') }}" class="text-sm text-text-light hover:text-dark transition-colors">Devenir Grossiste</a>
      <a href="{{ url('avantage-en-ligne') }}" class="text-sm text-text-light hover:text-dark transition-colors">Avantage en ligne</a>
      <a href="{{ url('contact') }}" class="text-sm text-text-light hover:text-dark transition-colors">Où nous trouver ?</a>
    </div>
  </div>
</div>
