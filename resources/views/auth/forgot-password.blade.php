...
<div class="auth-split">
  ...
  <div class="auth-form-container">
    <a href="{{ url('connexion') }}" class="back-home">
    ...
    <div class="auth-form fade-in-up delay-1">
      ...
      @endif
      @if(session('reset_url') && app()->environment('local'))
      <div style="padding:16px;background:rgba(196,150,122,0.1);border:1px dashed var(--color-warm);border-radius:var(--radius-md);color:var(--color-dark);margin-bottom:16px;font-size:14px;word-break:break-all;line-height:1.8;">
        ...
        <a href="{{ session('reset_url') }}" style="color:var(--color-warm);">{{ session('reset_url') }}</a>
      </div>
      @endif
      ...
      <form method="POST" action="{{ url('mot-de-passe-oublie') }}" id="forgot-form">
        @csrf
...
if ({{ session('email_sent', false) ? 'true' : 'false' }}) {
  showSlide(1);
}
</script>
</div>
